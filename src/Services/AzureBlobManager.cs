...
internal class AzureBlobManager : IAzureBlobManager
{
    ...
    protected async Task CreateEntity(CreateParameters createParameters, CancellationToken cancellationToken)
    {
        ...
        {
            throw new Exception(string.Format(Resources.SomethingWrongHappenedDuringProcessingExistingBlob, path));
        }
    }

    protected async Task WriteEntity(WriteParameters writeParameters, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
    ...
    protected PluginContext CreateContextFromStringData(string path, string data)
    {
    ...
    protected async Task WriteEntityFromContext(string path, PluginContext context, bool overwrite,
    ...
    protected async Task<PluginContext> ReadEntity(ReadParameters readParameters,
        CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
    ...
    protected async Task DeleteEntity(DeleteParameters deleteParameters, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
    ...
    protected async Task PurgeEntity(PurgeParameters purgeParameters, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        ...
        if (PathHelper.IsRootPath(path))
            throw new Exception(string.Format(Resources.TheEnteredPathMustBeNotRootPath, path));
        ...
        BlockBlobClient blockBlobClient = container.GetBlockBlobClient(path);
        var response = await blockBlobClient.DeleteIfExistsAsync(cancellationToken: cancellationToken);
        if (response.Value is false)
            throw new Exception(string.Format(Resources.TheEnteredPathCouldNotBePurged, path));
    }

    protected async Task<bool> ExistEntity(ExistParameters existParameters, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
    ...
    protected async Task<List<PluginContext>> ListEntities(ListParameters listParameters, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
    ...
    protected async Task<List<PluginContext>> ListBlobs(BlobContainerClient containerClient, 
    ...
    protected string? FormatFolderPrefix(string folderPath)
    {
    ...
    protected async Task<BlobContainerClient> GetBlobContainerClient()
    {
